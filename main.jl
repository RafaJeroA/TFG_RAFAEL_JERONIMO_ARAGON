...
function precargar_politicas_en_escenario!(
    ...
    for row in eachrow(politicas)
        ...
        if (lowercase(esc.nombre) == "bau") || (lowercase(esc.nombre) == "BAUSinCierre")
            p_co2   = 0.0
            subvmax = 0.0
        end    

        if (lowercase(esc.nombre) == "esfuerzo_x2") || (lowercase(esc.nombre) == "Esfuerzo_x2SinCierre")
            p_co2   *= 2.0
            subvmax *= 2.0
